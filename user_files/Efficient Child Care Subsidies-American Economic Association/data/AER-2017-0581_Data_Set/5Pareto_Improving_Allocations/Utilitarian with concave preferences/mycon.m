...
for r=2:N
    for s=1:N-1
        if r+s<N+2
            O(N*r-2*N+s)=((log(x(r-1,1))-(1/theta)*((((x(r-1,3)/z(r+s-2))+x(r-1,2))^(1+gamma))/(1+gamma))))-((log(x(r+s-1,1))-(1/theta)*((((x(r+s-1,3)/z(r+s-2))+x(r+s-1,2))^(1+gamma))/(1+gamma))));
        else
            O(N*r-2*N+s)=0;
        end
    end  
...
%% Pareto improvement
for r=1:1
    F(r)=-(log(x(r,1))-(1/theta)*(((x(r,2))^(1+gamma))/(1+gamma))-(Au1(r)/((10*n*50)/1000)));
end
for r=2:N
    F(r)=-(log(x(r,1))-(1/theta)*((((x(r,3)/z(r-1))+x(r,2))^(1+gamma))/(1+gamma))-(Au1(r)/((10*n*50)/1000)));
end
